...
def ida_star_search(start, goal, grid): # 'goal' ở đây sẽ được hàm heuristic lồng sử dụng
    ...
    # --- Phần chính của IDA* ---
    if not is_safe(start[0], start[1], grid) or not is_safe(goal[1], goal[2], grid):
        return None 
    ...
    while True:
        ...
        active_bound = status
DEBUG_SA_PATH = False
DEBUG_SA_GET_MOVES = False

# -----------------------------------------------------------------------------
# HÀM IS_SAFE GỐC - Dùng cho ô ĐÍCH của tất cả các bước đi
# -----------------------------------------------------------------------------
def is_safe(x, y, grid):
    # (Phiên bản này sử dụng kích thước thực của grid để kiểm tra biên)
    # if DEBUG_SA_PATH or DEBUG_SA_GET_MOVES: print(f"--- is_safe called with x={x}, y={y} ---")
    if not grid: return False
    actual_rows = len(grid)
    if actual_rows == 0: return False
    actual_cols = len(grid[0])
    if not (0 <= y < actual_rows and 0 <= x < actual_cols):
        # if DEBUG_SA_PATH or DEBUG_SA_GET_MOVES: print(f"    is_safe: Out of bounds ({x},{y}). actual_dims: ({actual_cols},{actual_rows}). Returning False.")
        return False
    val = grid[y][x]
    if val in (9, 10): return False # Nước
    if y == actual_rows - 1 and val == -1: return False # Hố hàng cuối
    # if DEBUG_SA_PATH or DEBUG_SA_GET_MOVES: print(f"    is_safe: ({x},{y}) val {val} -> Returning True")
    return True

# -----------------------------------------------------------------------------
# HÀM IS_SAFE CHO Ô TRUNG GIAN CỦA NHẢY (DÙNG CHO UCS và SA)
# -----------------------------------------------------------------------------
def is_safe_for_ucs_intermediate_jump_check(x, y, grid): # Tên hàm có thể đổi thành ...intermediate_jump_check
    # Giả định x, y đã trong biên (được kiểm tra bởi hàm gọi nó)
    val = grid[y][x]
    # if DEBUG_SA_PATH or DEBUG_SA_GET_MOVES: print(f"    is_safe_for_intermediate_jump_check ({x},{y}) val:{val}", end="")
    if val in (9, 10, -1): # Nước HOẶC BẤT KỲ Ô HỐ NÀO (-1)
        # if DEBUG_SA_PATH or DEBUG_SA_GET_MOVES: print(f" -> Is pit/water, RETURNING False (can jump over)")
        return False # "Không an toàn để đứng" -> "có thể nhảy qua ô này"
    # if DEBUG_SA_PATH or DEBUG_SA_GET_MOVES: print(f" -> Is solid ground, RETURNING True (cannot jump over)")
    return True # "An toàn để đứng" (đất liền) / "Vật cản" -> "KHÔNG thể nhảy qua ô này"

# -----------------------------------------------------------------------------
# HÀM ĐÁNH GIÁ TRẠNG THÁI (NĂNG LƯỢNG)
# -----------------------------------------------------------------------------
